...
- Global toggle to easily disable/enable sync across all buffers
...
#+BEGIN_SRC emacs-lisp
((nil . ((unison-sync-root1 . "/path/to/local/directory")
         (unison-sync-root2 . "/path/to/remote/directory")
...
Once configured, ~unison-sync-mode~ will be automatically enabled for files in directories where ~unison-sync-root1~ and ~unison-sync-root2~ are set.
...
- ~unison-sync-mode~: Toggle the minor mode on/off manually for the current buffer.
- ~unison-sync-force~: Force a sync operation with the -ignorearchives flag.
- ~unison-sync-global-mode~: Toggle unison-sync-mode globally across all buffers.

* Customization

You can customize the following variables:

- ~unison-sync-one-way-sync~: Set to non-nil to perform one-way synchronization from ~unison-sync-root1~ to ~unison-sync-root2~.
...
- ~unison-sync-global-mode-enabled~: Controls whether unison-sync should be enabled across all buffers. Normally you should use the ~unison-sync-global-mode~ command to toggle this.

Example:

#+BEGIN_SRC emacs-lisp
(setq unison-sync-one-way-sync t)
...
All PRs will automatically be tested in CI on GitHub.
