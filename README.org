...
- ~unison-global-mode~: Toggle unison-sync-mode globally across all buffers.
...
- ~unison-global-mode-enabled~: Controls whether unison-sync should be enabled across all buffers. Normally you should use the ~unison-global-mode~ command to toggle this.
